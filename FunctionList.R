...
rm.extra.comma=function(x){
  auths=x
  #auths=split.auths(x)
  auths=str_split(auths,";")[[1]] #make change by xiao 
  ncoms=str_count(auths,",") #make change by xiao
  ...
  #for (i in ncoms){
    ...
    if(class(json_data)!="try-error"){
      if(!is.null(json_data$message$author)){
        ...
        if(length(names_sep)==length(auths)){        #make change by xiao 
          auths[i]=names_sep[i]
        }else{
...
match.initials=function(x,allfirsts,alllasts,initials){
  ...
  if(initials[x]==F){
    ...
    others=which(tolower(target.last)==tolower(alllasts) & initials==F)
    if(length(others)>0){
...
match.common=function(x,namegends_2,commonnames){
  this_name=namegends_2$name[x]
  cn_index=which(commonnames$name==this_name)
  if(length(cn_index)>0){
    return(commonnames[cn_index,])
  }else{
    return(namegends_2[x,])
  }
}

#################################Step6_BuildRaceData.R########################
#N/A
...
citeprops=function(citeprops,i=NULL,type='conditional'){ 
  ...
  if(is.null(journal)){
    ...
    for(i in unique(article.data$PY)){
      ...
      timedata.i=data.frame(Year=rep(i,4),Gender=c("MM","WM","MW","WW"),
                            Prop=c(length(subgends[subgends=="MM"])/length(subgends),
                                   length(subgends[subgends=="WM"])/length(subgends),
                                   length(subgends[subgends=="MW"])/length(subgends),
                                   length(subgends[subgends=="WW"])/length(subgends)),
