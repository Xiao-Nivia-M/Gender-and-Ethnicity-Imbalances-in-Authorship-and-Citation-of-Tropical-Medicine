...
nicknames=as.matrix(read.csv("Src/nicknames.csv",header=F))
...
nickname.gends=read.csv("Src/nickname.gends.csv",header=T,stringsAsFactors=F)[,-1]
...
# Can substitute Utzigner for any name in your dataset that had variants
sort(allfirsts[alllasts=="Abbasi"])
sort(allfirsts_matched[alllasts=="Abbasi"])
